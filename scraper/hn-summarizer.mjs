 ...
 *
 * 全文翻译不在批处理里做（免费模型限流严重，批量翻译整版榜单性价比极低）：
 ...
 *
 * 环境变量（LLM 部分同 tagger/github-summarizer）: ZHIPU_API_KEY / ZHIPU_MODEL / ZHIPU_BASE_URL
 */
...
async function main() {
  ...
  console.log(
    `${PREFIX} 共 ${allIds.size} 条，摘要库已有 ${store.size} 条（清理过期 ${pruned} 条），` +
      `待处理 ${batch.length} 条（需摘要 ${toSummarize}，补存原文 ${toUpgrade}），模型 ${MODEL}`,
  );
  if (batch.length === 0) {
    ...
    if (pruned > 0) await saveSummaryStore(store, { model: MODEL, source: 'hn' });
  ...
  // 阶段二：LLM 摘要（只有缺摘要的条目花钱），连同原文/链接一起落库
  let done = 0;
  let failed = 0;
  await runPool(batch, API_CONCURRENCY, async (it) => {
    if (it.needsSummary) {
      try {
        const { title_zh, summary } = await translateAndSummarize(it, it.content, apiKey);
        it.entry = {
          title_zh,
          summary,
          lang: 'zh',
    ...
    if (done % 5 === 0) await saveSummaryStore(store, { model: MODEL, source: 'hn' });
  });

  if (done > 0 || pruned > 0) await saveSummaryStore(store, { model: MODEL, source: 'hn' });
