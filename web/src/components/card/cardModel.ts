...
/*
 * FeedCard 的视图模型（纯函数，无 JSX）—— mobile/src/components/card/cardModel.ts 的镜像副本。
 *
 ...
 *
 * 各源差异有两个合法出口，都不用改卡片：
 *   1. 数据差异   → data/normalize.ts 适配器映射成 TimelineItem 的公共字段；
 *   2. 长相差异   → data/sources.ts 注册表里该源的 card: CardVisual 微调（封面比例等）。
 *
 * ⚠️ 一卡到底约定：源内所有子板块（zhihu:recommend/follow/hot、bilibili:popular/rank…）
 *    以及未来任何新数据源，一律由 FeedCard 渲染，禁止为新源另写卡片组件。
 *    ⚠️ 改本文件必须同步 mobile 端镜像（安卓端是权威版本）。
 */
