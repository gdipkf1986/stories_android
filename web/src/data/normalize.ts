...
/** 源 0：知乎抓取数据（zhihu-scraper 的输出，recommend / follow / hot 多个流合并） */
export function normalizeZhihuFeed(raw: unknown): TimelineItem[] {
  ...
  for (const feed of feeds) {
    for (const entry of asArray(asDict(feed).items)) {
      ...
      const aiTags = asArray(it.tags).map((t) => str(t)).filter(Boolean);
...
/** B站条目类型 → 动作文案 + 标签（mobile/src/api/normalize.ts 的镜像逻辑） */
...
/** B站封面图规整：http → https（图床支持 https），拼官方缩略参数 @480w_270h.webp 省流量 */
